...
Drei Bereiche:
  Wuerfeln    Rezept waehlen, Menge skalieren, wuerfeln
  Fragen      Freitext-Chat mit Claude ueber die gesamte Sammlung
  Upload      Verarbeitung steuern (Segment "Verarbeiten") und Fehlerfaelle
              nacharbeiten (Segment "Fehler")
...
def hero(eyebrow: str, titel: str, unterzeile: str) -> None:
    st.markdown(
        f"""<div class="hero">
              ...
              <h1>{e(titel)}</h1>
              <p class="sub">{e(unterzeile)}</p>
            </div>""",
        unsafe_allow_html=True,
    )


def statistik_reihe(rezepte: list[Rezept]) -> None:
    verarbeitet = sum(1 for r in rezepte if r.status == STATUS_PROCESSED)
    offen = sum(1 for r in rezepte if r.status == STATUS_UNPROCESSED)
    fehler = sum(1 for r in rezepte if r.status == STATUS_ERROR)

    st.markdown(
        f"""<div class="stat-reihe">
              <div class="stat"><div class="zahl">{len(rezepte)}</div><div class="label">Rezepte gesamt</div></div>
              <div class="stat"><div class="zahl">{verarbeitet}</div><div class="label">kochbereit</div></div>
              <div class="stat"><div class="zahl">{offen}</div><div class="label">offen</div></div>
              <div class="stat"><div class="zahl">{fehler}</div><div class="label">Fehler</div></div>
            </div>""",
        unsafe_allow_html=True,
    )
...
def sidebar(rezepte: Optional[list[Rezept]]) -> None:
    with st.sidebar:
        ...
        if rezepte:
            kochbereit = sum(1 for r in rezepte if r.ist_skalierbar)
            st.markdown(
                f'<p class="muted">{len(rezepte)} Rezepte geladen · '
                f"{kochbereit} kochbereit</p>",
                unsafe_allow_html=True,
            )
...
def main() -> None:
    ...
    if SCHREIBZUGRIFF:
        kochen, fragen, upload = st.tabs(["Würfeln", "Fragen", "Upload"])
    else:
        # Oeffentliches Deployment: nur lesende Reiter.
        kochen, fragen = st.tabs(["Würfeln", "Fragen"])
        upload = None
